...
eeg_train = eeg_train1.reshape(eeg_train1.shape[0],-1)
...
# Save Directory
weights_save_dir = f'/mnt/data12_16T/tanaya/data/eeg_dataset/eeg/regression_weights/sub-{sub:02d}/'
...
save_dir = f'/mnt/data12_16T/tanaya/data/eeg_dataset/eeg/predicted_embeddings/sub-{sub:02d}/'
...
# VDVAE Regression
train_latents= np.load('/mnt/data12_16T/tanaya/results/thingseeg2_extracted_embeddings/train_autokl.npy', mmap_mode='r')[ids]
test_latents = np.load('/mnt/data12_16T/tanaya/results/thingseeg2_extracted_embeddings/test_autokl.npy', mmap_mode='r')
print(train_latents.shape, test_latents.shape)

print("Training VDVAE Regression")
reg = skl.Ridge(alpha=3000, max_iter=50000, fit_intercept=True) # alpha=50000
...
train_clip_all = np.load('/mnt/data12_16T/tanaya/results/thingseeg2_extracted_embeddings/train_clipvision.npy', mmap_mode='r')
test_clip = np.load('/mnt/data12_16T/tanaya/results/thingseeg2_extracted_embeddings/test_clipvision.npy', mmap_mode='r')
...
for i in range(num_token):
    ...
    reg = skl.Ridge(alpha=2000, max_iter=50000, fit_intercept=True) # alpha=60000
...
train_clip_all = np.load('/mnt/data12_16T/tanaya/results/thingseeg2_extracted_embeddings/train_cliptext.npy', mmap_mode='r')
test_clip = np.load('/mnt/data12_16T/tanaya/results/thingseeg2_extracted_embeddings/test_cliptext.npy', mmap_mode='r')
train_clip = train_clip_all[ids]
...
pred_clip = np.zeros_like(test_clip)
for i in range(num_token):
    reg = skl.Ridge(alpha=20000, max_iter=50000, fit_intercept=True) # old alpha=100000
    reg.fit(eeg_train, train_clip[:,i])
