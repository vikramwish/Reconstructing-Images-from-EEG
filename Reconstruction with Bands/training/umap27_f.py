...
def load_word_mapping(xlsx_path):
    """Load the Excel file containing word to uniqueID mapping"""
    df = pd.read_excel(xlsx_path)
    print(f"Loaded word mapping file with shape: {df.shape}")
    print(f"Columns: {df.columns.tolist()}")

    # Create bidirectional mapping
    word_to_uniqueid = dict(zip(df['Word'], df['uniqueID']))
    uniqueid_to_word = dict(zip(df['uniqueID'], df['Word']))

    return word_to_uniqueid, uniqueid_to_word, df


def load_categories_matrix(tsv_path):
    """Load the TSV file containing category matrix (one-hot encoded)"""
    df = pd.read_csv(tsv_path, sep='\t')
    print(f"Loaded categories matrix with shape: {df.shape}")
    print(f"Category columns: {df.columns.tolist()}")
    print(f"Number of categories: {len(df.columns)}")

    return df


def create_category_mappings(word_df, category_df):
    """Create mappings from words/uniqueIDs to their categories"""
    # Ensure both dataframes have the same number of rows
    if len(word_df) != len(category_df):
        raise ValueError(f"Mismatch in number of rows: word_df has {len(word_df)}, category_df has {len(category_df)}")

    word_to_category = {}
    uniqueid_to_category = {}

    # Get category names (column names from category matrix)
    category_names = category_df.columns.tolist()

    for i in range(len(word_df)):
        word = word_df.iloc[i]['Word']
        uniqueid = word_df.iloc[i]['uniqueID']

        # Find which category has value 1 for this row
        category_row = category_df.iloc[i]
        category_indices = np.where(category_row == 1)[0]

        if len(category_indices) == 0:
            # No category assigned (all zeros)
            category = 'unknown'
        elif len(category_indices) == 1:
            # Single category assigned
            category = category_names[category_indices[0]]
        else:
            # Multiple categories assigned - take the first one or handle as needed
            category = category_names[category_indices[0]]
            print(
                f"Warning: Multiple categories found for {word}/{uniqueid}: {[category_names[idx] for idx in category_indices]}")

        word_to_category[word] = category
        uniqueid_to_category[uniqueid] = category

    return word_to_category, uniqueid_to_category, category_names


def load_categories_mapping(xlsx_path, tsv_path):
    """Load both files and create comprehensive word to category mapping"""
    # Load word mapping
    word_to_uniqueid, uniqueid_to_word, word_df = load_word_mapping(xlsx_path)

    # Load category matrix
    category_df = load_categories_matrix(tsv_path)

    # Create category mappings
    word_to_category, uniqueid_to_category, unique_categories = create_category_mappings(word_df, category_df)

    print(f"Created mappings for {len(word_to_category)} words")
    print(f"Number of unique categories: {len(unique_categories)}")

    return word_to_category, uniqueid_to_category, unique_categories, word_df, category_df
    ...
    for cat in present_categories:
        ...
        if len(cat_indices) > 0:
            ...
            p.square(embedding_2d[cat_indices, 0], embedding_2d[cat_indices, 1],
                     size=15, color=color, alpha=0.8, line_color='black', line_width=1,
...
def create_matplotlib_2d_umap(embedding_2d, test_categories, test_images,
    ...
    plt.tight_layout()
    plt.savefig(f'{save_path}/matplotlib_2d.png', dpi=300, bbox_inches='tight')
    plt.savefig(f'{save_path}/matplotlib_2d.pdf', bbox_inches='tight')
...
# Load categories mapping from Excel and TSV files
xlsx_path = '/mnt/data12_16T/tanaya/data/thingseeg2_metadata/imageword27.xlsx'  
tsv_path = '/mnt/data12_16T/tanaya/data/thingseeg2_metadata/category27_manual.tsv'  
word_to_category, uniqueid_to_category, unique_categories, word_df, category_df = load_categories_mapping(xlsx_path, tsv_path)
...
# Generate colors for ALL 27 categories (not just detected ones)
print(f"\nGenerating colors for ALL {len(unique_categories)} categories...")
...
# Set up save paths
save_base_path = '/mnt/data12_16T/tanaya/data/eeg_dataset/results_5/thingseeg2_preproc/sub-08/umaps27'
