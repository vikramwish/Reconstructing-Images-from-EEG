...
parser.add_argument('-duration', '--duration', help='Duration', default=40)
parser.add_argument('-dnn', '--using_dnn', help='Using Deep Neural Networks', default=False, action=argparse.BooleanOptionalAction)
...
pred_autokl = np.load(f'/mnt/data12_16T/tanaya/data/eeg_dataset/higamma/predicted_embeddings/sub-{sub:02d}/regress_vdvae{param}.npy')
pred_cliptext = np.load(f'/mnt/data12_16T/tanaya/data/eeg_dataset/higamma/predicted_embeddings/sub-{sub:02d}/regress_cliptext{param}.npy')
pred_clipvision = np.load(f'/mnt/data12_16T/tanaya/data/eeg_dataset/higamma/predicted_embeddings/sub-{sub:02d}/regress_clipvision{param}.npy')

if using_dnn:
    pred_clipvision = np.load(f'/mnt/data12_16T/tanaya/data/eeg_dataset/alpha/predicted_embeddings/sub-{sub:02d}/dnn_clipvision{param}.npy')
...
print(param)
vdvae_recon_dir = f'/mnt/data12_16T/tanaya/data/eeg_dataset/foundation_eeg/thingseeg2_preproc/vdvae{param}/'
diffusion_recon_dir = f'/mnt/data12_16T/tanaya/data/eeg_dataset/foundation_eeg/thingseeg2_preproc/versatile_diffusion{param}/'
if using_dnn:
    diffusion_recon_dir = f'/mnt/data12_16T/tanaya/data/eeg_dataset/results_hgmm/thingseeg2_preproc/sub-{sub:02d}/versatile_diffusion{param}_dnn/'
...
# %%
import sys
sys.path.append('/mnt/data12_16T/tanaya/EEG-Image-Reconstruction-main/vdvae')
import torch
import numpy as np
...
H = {'image_size': 64, 'image_channels': 3,'seed': 0, 'port': 29500, 'save_dir': '/mnt/data12_16T/tanaya/EEG-Image-Reconstruction-main/vdvae/saved_models/test', 'data_root': '/mnt/data12_16T/tanaya/EEG-Image-Reconstruction-main/vdvae', 'desc': 'test', 'hparam_sets': 'imagenet64', 'restore_path': '/mnt/data12_16T/tanaya/EEG-Image-Reconstruction-main/vdvae/model/imagenet64-iter-1600000-model.th', 'restore_ema_path': '/mnt/data12_16T/tanaya/EEG-Image-Reconstruction-main/vdvae/model/imagenet64-iter-1600000-model-ema.th', 'restore_log_path': '/mnt/data12_16T/tanaya/EEG-Image-Reconstruction-main/vdvae/model/imagenet64-iter-1600000-log.jsonl', 'restore_optimizer_path': '/mnt/data12_16T/tanaya/EEG-Image-Reconstruction-main/vdvae/model/imagenet64-iter-1600000-opt.th', 'dataset': 'imagenet64', 'ema_rate': 0.999, 'enc_blocks': '64x11,64d2,32x20,32d2,16x9,16d2,8x8,8d2,4x7,4d4,1x5', 'dec_blocks': '1x2,4m1,4x3,8m4,8x7,16m8,16x15,32m16,32x31,64m32,64x12', 'zdim': 16, 'width': 512, 'custom_width_str': '', 'bottleneck_multiple': 0.25, 'no_bias_above': 64, 'scale_encblock': False, 'test_eval': True, 'warmup_iters': 100, 'num_mixtures': 10, 'grad_clip': 220.0, 'skip_threshold': 380.0, 'lr': 0.00015, 'lr_prior': 0.00015, 'wd': 0.01, 'wd_prior': 0.0, 'num_epochs': 10000, 'n_batch': 4, 'adam_beta1': 0.9, 'adam_beta2': 0.9, 'temperature': 1.0, 'iters_per_ckpt': 25000, 'iters_per_print': 1000, 'iters_per_save': 10000, 'iters_per_images': 10000, 'epochs_per_eval': 1, 'epochs_per_probe': None, 'epochs_per_eval_save': 1, 'num_images_visualize': 8, 'num_variables_visualize': 6, 'num_temperatures_visualize': 3, 'mpi_size': 1, 'local_rank': 0, 'rank': 0, 'logdir': '/mnt/data12_16T/tanaya/EEG-Image-Reconstruction-main/vdvae/model/saved_models/test/log'}
class dotdict(dict):
...
# %%
import sys
sys.path.append('/mnt/data12_16T/tanaya/EEG-Image-Reconstruction-main/versatile_diffusion')
import os
...
pth = '/mnt/data12_16T/tanaya/EEG-Image-Reconstruction-main/versatile_diffusion/pretrained/vd-four-flow-v1-0-fp16-deprecated.pth'
