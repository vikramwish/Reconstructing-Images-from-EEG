...
def epoching(args, data_part, seed):
    ...
    for s in range(args.n_ses):
        ...
        # Filter the raw data in bands , uncomment line which is required
        #filtered_raw = raw.copy().filter(l_freq=8, h_freq=13, fir_design='firwin', verbose=True)
        #filtered_raw = raw.copy().filter(l_freq=0.5, h_freq=4, fir_design='firwin', verbose=True)
        # filtered_raw = raw.copy().filter(l_freq=14, h_freq=30, fir_design='firwin', verbose=True)
        #filtered_raw = raw.copy().filter(l_freq=5, h_freq= 7, fir_design='firwin', verbose=True)
        #gamma
        filtered_raw = raw.copy().filter(l_freq=30, h_freq= 100, fir_design='firwin', verbose=True)
        # Get events
