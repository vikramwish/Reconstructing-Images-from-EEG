...
def epoching(args, data_part, seed):
    ...
    for s in range(args.n_ses):
        ...
        #*** Select the channels here, uncomment each combination for a training
        ...
        #17 electrodes
        #specific_channels = np.asarray(mne.pick_channels_regexp(raw.info['ch_names'],'^O[0-9z]|^PO[0-9z]|^P[0-9z]'))
        # 8 channels own selection parieto-occipital regions
        #specific_channels = np.asarray(mne.pick_channels_regexp(raw.info['ch_names'],
        #                                               '^PO[0-9z]|^O[0-9z]'))
