...
class EEGViTDataLoader:
    ...
    def __init__(self, harmonized_dirs: Union[Path, List[Path]], batch_size: int = 16,
        ...
        # # Find all harmonized files - EXACT from working version
        # self.harmonized_files = []
        # for dataset_dir in self.harmonized_dir.iterdir():
        #     if dataset_dir.is_dir():
        #         files = list(dataset_dir.glob("*_harmonized.h5"))
        #         self.harmonized_files.extend(files)
        #
        # logger.info(f"Found {len(self.harmonized_files)} harmonized files")
        #
        # # ADDED: Build category mappings
        # self.category_mappings = {'category_to_id': {}, 'super_category_to_id': {}}
        # self._build_category_mappings()
        #
        # # Analyze data dimensions - EXACT from working version
    ...
    #         # Find max sequence length across all files
    #         max_seq_len = 0
    #         for file_path in self.harmonized_files:
    #             with h5py.File(file_path, 'r') as f:
    #                 sample_data = f['embeddings_sequence']
    #                 _, seq_len, embed_dim = sample_data.shape
    #                 max_seq_len = max(max_seq_len, seq_len)
    #
    #         # Use first file for trials and embedding dim, max for sequence length
    #         with h5py.File(self.harmonized_files[0], 'r') as f:
    #             sample_data = f['embeddings_sequence']
    #             self.n_trials, _, self.embedding_dim = sample_data.shape
    #             self.sequence_length = max_seq_len  # Use maximum across all files
    #
    #         logger.info(
    #             f"Data dimensions: {self.n_trials} trials, {self.sequence_length} seq_len (max), {self.embedding_dim} embed_dim")
    def _analyze_data_dimensions(self):
        """Analyze data dimensions"""
        if self.harmonized_files:
            max_seq_len = 0
    ...
    #     """Get data config with category info """
    #     return {
    #         'input_trials': self.n_trials,
    #         'input_sequence_len': self.sequence_length,
    ...
    #     }
    #
    #     logger.info(f"Data config: {config}")
    #     return config
    def get_data_config(self) -> Dict[str, int]:
        """Get data config with unified category info"""
...
class MultiGPUEEGViTTrainer:
    ...
    #     self.config = config
    #
    #     # Multi-GPU setup - EXACT from working version
    ...
    #
    #     # Multi-GPU setup with proper model placement
    #     self.device = torch.device('cuda:0')
    ...
    #         logger.info(f"Model distributed across GPUs: {[0, 1, 2, 3]}")
    #     else:
    ...
    #         lr=1e-5,  # MUCH lower - was 2e-4
    #         #lr=1e-3,
    #         weight_decay=1e-5,  # Lower weight decay too
    #         #weight_decay = 1e-4,
    #         betas=(0.9, 0.999),
    #         #betas = (0.9, 0.95)
    #     )
    #
    #     # # Learning rate scheduler - EXACT from working version
    ...
    #         'initial_temp': 1.5,
    #         'final_temp': 0.3,
    #         'anneal_epochs': 50,  # Anneal over first 50 epochs
    #         'strategy': 'cosine'  # or 'linear'
    #     }
    #
    #     # SPEED UP: Better scheduler
    #     # self.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
    #     #     self.optimizer, T_max=50, eta_min=1e-5  # Faster annealing
    #     # )
    #     # Loss functions - EXACT from working version
    ...
    def __init__(self, model: EEGViTCNetSelfAware, config: EEGViTCNetConfig):
        ...
        # Multi-GPU setup - MUST happen before .to(device)
        # if torch.cuda.is_available() and torch.cuda.device_count() >= 2:
        #     logger.info(f"Found {torch.cuda.device_count()} GPUs")
        #     self.model = nn.DataParallel(model, device_ids=[0, 1, 2, 3])
        #     logger.info(f"DataParallel enabled on GPUs: {list(range(torch.cuda.device_count()))}")
        # else:
        #     self.model = model
        ...
        #     logger.info(f"DataParallel confirmed. Batch will be split across {len(self.model.device_ids)} GPUs")

        # Optimizer Use self.model.parameters() not model.parameters()
        self.optimizer = torch.optim.AdamW(
    ...
    #     """Compute loss with category support """
    #
    #     # Fix the circular reconstruction loss 
    #     if targets is None:
    #         # Use temporal features as reconstruction target (not combined_features)
    #         reconstruction_target = outputs['temporal_features'].detach()  # Stop gradients
    #         reconstruction_loss = self.reconstruction_loss(
    #             outputs['reconstruction'][:, :reconstruction_target.shape[1]],  # Match dimensions
    ...
    #         )
    #
    #     # adding Category losses
    #     category_loss = torch.tensor(0.0, device=self.device)
    #     super_category_loss = torch.tensor(0.0, device=self.device)
    #
    #     if category_ids is not None and mapped_mask is not None and mapped_mask.any():
    #         valid_indices = mapped_mask.nonzero().squeeze(-1)
    #         if len(valid_indices) > 0:
    #             pred_cat = outputs['predicted_categories'][valid_indices]
    #             true_cat = category_ids[valid_indices]
    #             category_loss = F.cross_entropy(pred_cat, true_cat)
    #
    #     if super_category_ids is not None and mapped_mask is not None and mapped_mask.any():
    #         valid_indices = mapped_mask.nonzero().squeeze(-1)
    #         if len(valid_indices) > 0:
    #             pred_super = outputs['predicted_super_categories'][valid_indices]
    #             true_super = super_category_ids[valid_indices]
    #             super_category_loss = F.cross_entropy(pred_super, true_super)
    #
    #         # checking Check super-category loss computation
    #     if super_category_ids is not None and mapped_mask is not None:
    #         logger.info(f"Debug super-category loss:")
    #         logger.info(f"  mapped_mask sum: {mapped_mask.sum().item()}")
    #         logger.info(f"  super_category_ids shape: {super_category_ids.shape}")
    #         logger.info(f"  unique super_category_ids: {torch.unique(super_category_ids)}")
    #
    #         if mapped_mask.any():
    #             valid_indices = mapped_mask.nonzero().squeeze(-1)
    ...
    #                 pred_super = outputs['predicted_super_categories'][valid_indices]
    #                 true_super = super_category_ids[valid_indices]
    #                 logger.info(f"  pred_super shape: {pred_super.shape}")
    #                 logger.info(f"  true_super: {true_super}")
    #                 super_category_loss = F.cross_entropy(pred_super, true_super)
    #             else:
    #                 logger.warning("No valid indices for super-category loss!")
    #         else:
    #             logger.warning("No mapped samples in batch!")
    #
    ...
    #
    #     # Total loss with category components
    #     total_loss = (
    #             reconstruction_loss +
    #             0.001 * load_balance_loss +  # Reduce from 0.01
    #             0.0001 * specialization_loss +  # Reduce from 0.001
    #             0.001 * category_loss +  # Reduce from 0.1 , Much lower - objects are hard
    #             0.5 * super_category_loss  # Reduce from 0.1, 1000x higher - focus on categories most
    #     )
    #
    #
    #     # Ensure all losses are scalars for DataParallel 
    #     return {
    ...
    def compute_loss(self, outputs: Dict[str, torch.Tensor],
        ...
        # Category losses (only compute if valid data)
        # category_loss = torch.tensor(0.0, device=self.device)
        # super_category_loss = torch.tensor(0.0, device=self.device)
        #
        # # Super category loss (focus as requested)
        # if super_category_ids is not None:
        #     valid_mask = super_category_ids >= 0  # Valid categories only
        #     if valid_mask.sum() > 0:
...
class FastEEGDataset(Dataset):
    ...
    #     # import time
    #     # logger.info("Testing data loader speed...")
    #     # start = time.time()
    #     # batch = next(iter(train_loader))
    #     # logger.info(f"First batch load time: {time.time() - start:.2f}s")
    #     file_path, trial_idx = self.samples[idx]
    #     h5_file = self.file_cache[file_path]
    #
    #     # Direct read from memory-mapped file - very fast
    #     seq = torch.from_numpy(h5_file["embeddings_sequence"][trial_idx]).float()
    #
    #     label = -1
    #     if "super_category_ids" in h5_file and trial_idx < len(h5_file["super_category_ids"]):
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Create data splits
        # if args.auto_split:
        #     train_files, val_files = create_subject_splits(data_loader.harmonized_files)
        # else:
        #     train_files, val_files = filter_subject_files_by_dataset(
        #         data_loader.harmonized_files,
        #         args.train_dataset, args.val_dataset,
        #         args.train_subjects, args.val_subjects
        #     )
        ...
        # train_dataset = ShardedPTDataset(
        #     pt_root="/home/tanaya/phase2_2/alljoined",  #  shard output directory
        #     max_trials_per_subject=None  # Use all trials
        # )
        #
        # train_loader = DataLoader(
        #     train_dataset,
        #     batch_size=256,
        #     shuffle=True,
        #     num_workers=8,  # Now works efficiently!
        #     pin_memory=True,
        ...
        for epoch in range(args.epochs):
            ...
            #
            #     seq_emb = seq_emb.to(trainer.device, non_blocking=True)
            #     labels_super = labels_super.to(trainer.device, non_blocking=True)
            #
            ...
            #
            #     # Forward
            #     outputs = model(seq_emb, super_category_ids=labels_super)
            #     loss_dict = trainer.compute_loss(outputs, super_category_ids=labels_super)
            #
            #     # Backward
